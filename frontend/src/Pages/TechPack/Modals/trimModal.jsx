...
const TrimModal = ({ isOpen, closeModal, onSubmit, techPackData }) => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
          try {
            const response = await axios.get("/api/trim/getnames");
      ...
      const handleCategoryChange = async (e) => {
        ...
        try {
          ...
          const response = await axios.get(`/api/trim/names?name=${category}`);
          console.error("Error fetching categorieszzzzzzzzzzzzzzzzzzz:",response.data.trims);
    ...
    return (
      <Modal isOpen={isOpen} onRequestClose={closeModal} ariaHideApp={false} style={modalStyles}>
              ...
              <div className="images">
                {images.length > 0 ? (
                  images.map((image, index) => (
                    <div key={index} className="image-item" style={imageItemStyle}>
                      <label>
                        <input
                          type="checkbox"
                        ...
                        />
                        <img src={image.previewImage} alt={`image-${index}`} style={imageStyle} />
                      </label>
                    </div>
                  ))
                ) : (
